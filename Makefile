...
# targets
# build generateconfig-env image only (no container run, no cleanup)
build-env:
ifeq ($(QUIET_MODE),true)
	docker buildx build --quiet --load --tag generateconfig-env --file Dockerfile-generateconfig-env .
else

...
# Сохраняем существующую сеть: если уже есть etc/client.yml, не перезаписываем etc/ (например, после восстановления из бэкапа)
if [[ -f "${DEST_PATH}/client.yml" ]]; then
    echo "INFO: ${DEST_PATH}/client.yml already exists, skipping config generation (preserving existing network)"
    exit 0
fi

...
Operational change on top of the original base: if `etc/client.yml` already exists, config generation does not overwrite `etc/`.

The entries below are the original changelog of [anyproto/any-sync-dockercompose](https://github.com/anyproto/any-sync-dockercompose).
